...
game.States.boot = function(){
	this.preload = function(){
		...
		game.load.image('loading','/static/assets/preloader.gif');
	};
...
game.States.preload = function(){
	this.preload = function(){
		...
		//以下为要加载的资源
		game.load.image('background','/static/assets/background.png'); //背景
    	game.load.image('ground','/static/assets/ground.png'); //地面

		game.load.image('sky','/static/assets/ground.png'); //sky?

    	game.load.image('title','/static/assets/title.png'); //游戏标题
    	game.load.spritesheet('bird','/static/assets/bird.png',34,24,3); //鸟
    	game.load.image('btn','/static/assets/start-button.png');  //按钮
    	game.load.spritesheet('pipe','/static/assets/pipes.png',54,320,2); //管道
    	game.load.bitmapFont('flappy_font', '/static/assets/fonts/flappyfont/flappyfont.png', '/static/assets/fonts/flappyfont/flappyfont.fnt');
    	game.load.audio('fly_sound', '/static/assets/flap.wav');//飞翔的音效
    	game.load.audio('score_sound', '/static/assets/score.wav');//得分的音效
    	game.load.audio('hit_pipe_sound', '/static/assets/pipe-hit.wav'); //撞击管道的音效
    	game.load.audio('hit_ground_sound', '/static/assets/ouch.wav'); //撞击地面的音效

    	game.load.image('ready_text','/static/assets/get-ready.png');
    	game.load.image('play_tip','/static/assets/instructions.png');
    	game.load.image('game_over','/static/assets/gameover.png');
    	game.load.image('score_board','/static/assets/scoreboard.png');
	}
	this.create = function(){
...
game.States.menu = function(){
	this.create = function(){
		...
		game.add.tileSprite(0,-112,game.width,112,'sky').autoScroll(100,0); //sky
...
game.States.play = function(){
	this.create = function(){
		...
		this.sky = game.add.tileSprite(0,-112,game.width,112,'sky'); //sky?
		...
		this.sky.body.immovable = true; //sky?
	...
	this.update = function(){
		...
		game.physics.arcade.collide(this.bird,this.sky, this.hitGround, null, this); //sky?
	...
	this.statrGame = function(){
		...
		this.sky.autoScroll(-this.gameSpeed,0);//sky?
	...
	this.showGameOverText = function(){
		...
		let result =  document.getElementById("flappy").textContent;
		game.bestScore = result || 0;
		...
		var gameOverText = this.gameOverGroup.create(game.width/2,0,'game_over'); //game_flappy over 文字图片
